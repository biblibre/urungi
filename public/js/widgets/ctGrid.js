app.service('crossTableGrid', function (dataElements) {
    this.getCrossTableGrid = async function (report, $templateRequest) {
        ...
        const columnId = report.properties.tableKeys.column.id;
        const rowId = report.properties.tableKeys.row.id;
    ...
    this.getParams = function (report) {
        
        const dimensions = {};

        for(col of report.properties.columns){
            dimensions[col.id] = {
                label : col.objectLabel
            }
        }

        const valueDataFields = [];

        for(ykey of report.properties.ykeys){
            valueDataFields.push(ykey.id);
        }

        console.log(report.query.data);

        const params = {
            configuration: false, 
            data: report.query.data,
            horizontalDimensions : [report.properties.tableKeys.row.id],
            verticalDimensions : [report.properties.tableKeys.column.id],
