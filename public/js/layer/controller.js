...
angular.module('app').controller('layerCtrl', function ($scope, $location, api, $routeParams, $timeout, $window, gettextCatalog, toastr) {
    ...
    $scope.editSQL = function () {
        ...
        if (!selectedCollection.isSQL) {
            toastr.error(gettextCatalog.getString('Cannot modify sql of an object which is not an sql request'));
            return;
        }
    ...
    $scope.addDatasetToLayer = function (datasourceID, collectionName) {
        if ($scope._Layer.datasourceID === datasourceID) {
            ...
            toastr.error(gettextCatalog.getString('Datasource must be the same for all entities'));
        }
    };
