(function () {
    ...
    DataSourcesEditController.$inject = ['$routeParams', '$location', 'gettextCatalog', 'notify', 'api', '$rootScope'];

    function DataSourcesEditController ($routeParams, $location, gettextCatalog, notify, api, $rootScope) {
        const vm = this;
        ...
        function save () {
            if (vm.mode === 'add') {
                api.createDatasource(vm._dataSource).then(data => {
                    notify.success(gettextCatalog.getString('Datasource created successfully'));
                ...
                api.updateDatasource(vm._dataSource._id, vm._dataSource).then(result => {
                    notify.success(gettextCatalog.getString('Datasource updated successfully'));
                    $location.url('/data-sources');
                });
            }
        };
