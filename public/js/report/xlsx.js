...
import { getColumnDescription, getColumnId } from './util.js';
export function saveReportAsXLSX (report, data) {
    ...
    return new Blob([s2ab(wbout)], { type: 'application/vnd.ms-excel' });
}
...
function sheet_from_array_of_arrays (report, data) {
    ...
    for (let i = 0; i < report.properties.columns.length; i++) {
        ...
        const cell = { v: getColumnDescription(report.properties.columns[i]) };
    ...
    for (let R = 0; R !== data.length; ++R) {
        for (let i = 0; i < report.properties.columns.length; i++) {
            ...
            let cell;
            const columnId = getColumnId(report.properties.columns[i]);
