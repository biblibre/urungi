(function () {
    ...
    function grid (moment, numeral, api, layerService, reportsService) {
            ...
            return api.getLayer(report.selectedLayerID).then(layer => {
                ...
                if (options.vertical) {
                    for (const row of rows) {
                        ...
                        for (const column of report.properties.columns) {
                            ...
                            html += '<td>';
                            html += formatValue(row[column.id], column, layerObjectsMap);
                            html += '</td>';
                            html += '</tr>';
                        }
                        html += '</tbody>';
                    ...
                    for (const row of rows) {
                        html += '<tr>';
                        for (const column of report.properties.columns) {
                            const value = formatValue(row[column.id], column, layerObjectsMap);
                            html += '<td>';
                            html += value;
                            html += '</td>';
                        }
                ...
                for (const column of report.properties.columns) {
                    if (column.calculateTotal && (column.elementType === 'number' || ['count', 'countDistinct'].includes(column.aggregation))) {
                        ...
                        totals.set(column.id, formatValue(total, column, layerObjectsMap));
                    }
                }
        ...
        function formatValue (value, column, layerObjectsMap) {
