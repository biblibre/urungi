...
LayerEditController.$inject = ['$scope', '$timeout', '$window', 'api'];

function LayerEditController ($scope, $timeout, $window, api) {
    const vm = this;
    $scope.items = [];
    $scope.datasources = [];
    ...
    $scope.save = function () {
        ...
        api.replaceLayer(theLayer).then(function () {
            ...
            notify.success(t('Layer saved successfully'));
        }).catch(() => {
            notify.error(t('Something went wrong, check the server logs'));
        });
    };
                ...
                try {
                    for (const el of layerUtils.getElementsUsedInCustomExpression(element.viewExpression, $scope._Layer)) {
                        ...
                        if (element.component !== undefined && element.component !== comp) {
                            element.component = -1;
                            const msg = t('One of the custom elements uses elements from tables which are not joined. This custom element cannot be fetched');
                            notify.notice(msg);
                            return;
                        }
                    ...
                    const message = t('Failed to parse expression for element') + ' ' +
    ...
    $scope.erDiagramInit = function () {
        ...
        // this timeout is here to give time to angular to create the element's divs'
            setTimeout(function () {
                ...
                const rightJoinType = {
                    overlays: [
                        ['Label', { location: 0.88, label: '[' + t('right') + ']', labelStyle: { cssClass: 'leftJoinType', color: '#000', font: 'bold 14px ER', fill: ' #fff no-repeat fixed center' } }]
                    ]
                ...
                const leftJoinType = {
                    overlays: [
                        ['Label', { location: 0.10, label: '[' + t('left') + ']', labelStyle: { cssClass: 'leftJoinType', color: '#000', font: 'bold 14px ER', fill: ' #fff no-repeat fixed center' } }]
                    ]
                };
    ...
    $scope.addFolder = function () {
        ...
        const element = {};
        element.elementLabel = t('my folder');
    ...
    $scope.addDatasetToLayer = function (datasourceID, collectionName) {
        if ($scope._Layer.datasourceID === datasourceID) {
            ...
            notify.error(t('Datasource must be the same for all entities'));
        }
    };
