exports.testConnection = function (req, data, done) {
    ...
    { var dbURI = 'mongodb://' + data.userName + ':' + data.password + '@' + data.host + ':' + data.port + '/' + data.database; } else { var dbURI = 'mongodb://' + data.host + ':' + data.port + '/' + data.database; }
...
exports.execOperation = function (operation, params, done) {
    ...
    DataSources.findOne({ _id: params.datasourceID}, function (err, dataSource) {
        if (dataSource) {
            ...
            { var dbURI = 'mongodb://' + dataSource.params[0].connection.userName + ':' + dataSource.params[0].connection.password + '@' + dataSource.params[0].connection.host + ':' + dataSource.params[0].connection.port + '/' + dataSource.params[0].connection.database; } else { var dbURI = 'mongodb://' + dataSource.params[0].connection.host + ':' + dataSource.params[0].connection.port + '/' + dataSource.params[0].connection.database; }
                ...
                for (var field in item) {
                    ...
                    for (var e in collection.columns) {
                        if (field == collection.columns[e].elementName ||
                            ...
                            if (collection.columns[e].aggregation) { var elementID = 'wst' + collection.columns[e].elementID.toLowerCase() + collection.columns[e].aggregation; } else { var elementID = 'wst' + collection.columns[e].elementID.toLowerCase(); }
