            ...
            for (var d = 0; d < result.rows.length; d++) {
                for (var s = 0; s < schemas.length; s++) {
                    if (schemas[s].schema_name == result.rows[d].table_schema) {
                        ...
                        if (result.rows[d].table_schema) { table.element_name = result.rows[d].table_schema + '.' + result.rows[d].table_name; } else { table.element_name = result.rows[d].table_name; }
                ...
                for (var d = 0; d < result2.rows.length; d++) {
                    for (var t = 0; t < tables.length; t++) {
                        if (tables[t].table_name == result2.rows[d].table_name) {
                            ...
                            if (result2.rows[d].table_schema) { column.element_name = result2.rows[d].table_schema + '.' + result2.rows[d].column_name; } else { column.element_name = result2.rows[d].column_name; }
...
function processCollections (req, query, collections, dataSource, params, thereAreJoins, setresult, index) {
    ...
    for (var f in fields) {
        if (f == fields.length - 1) { SQLstring = SQLstring + fields[f]; } else { SQLstring = SQLstring + fields[f] + ', '; }
    }
    ...
    getFilters(query, function (filtersResult, havingsResult) {
        ...
        if (groupBy.length > 0) { SQLstring = SQLstring + ' GROUP BY '; }

        for (var f in groupBy) {
            if (f == groupBy.length - 1) { SQLstring = SQLstring + groupBy[f]; } else { SQLstring = SQLstring + groupBy[f] + ', '; }
        }
            ...
            if (query.order.length > 0) {
                ...
                for (var f in query.order) {
                    ...
                    // The order by field is in the result set
                        if (theOrderByString == '') { theOrderByString += (theIndex + 1) + sortType; } else { theOrderByString += ', ' + (theIndex + 1) + sortType; }
                    } else {
                    // No index, the field is not in the result set
                        if (theOrderByString == '') { theOrderByString += theSortOrderFieldName + sortType; } else { theOrderByString += ', ' + theSortOrderFieldName + sortType; }
                    }
                }
...
function getFilterSQL (filter, isHaving) {
    ...
    if (((filter.filterText1 && filter.filterText1 != '') || filter.filterType == 'notNull' || filter.filterType == 'null')) {
        ...
        if (!filter.aggregation) { var filterElementName = filter.collectionID + '.' + filter.elementName; } else { var filterElementName = filter.aggregation + '(' + filter.collectionID + '.' + filter.elementName + ')'; }
        ...
        if (filter.filterType == 'in' && filter.elementType != 'date') {
            if (filter.elementType == 'number') {
                ...
                for (var s in theSplit) {
                    if (s == 0) { filterSTR += "'" + theSplit[s] + "'"; } else { filterSTR += ",'" + theSplit[s] + "'"; }
                }
...
function dateFilter (filterElementName, filterValue, filter) {
    ...
    if (filterValue == '#WST-THISMONTH#') {
        ...
        if (month == 12) { var lastDate = new Date((year + 1) + '-01-01T00:00:00.000Z'); } else {
    ...
    if (filterValue == '#WST-LASTMONTH#') {
        if (month == 1) { var firstDate = new Date((year - 1) + '-12-01T00:00:00.000Z'); } else {
